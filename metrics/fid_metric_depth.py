...
__all__ = ['FIDMetricDepth', 'FID50KDepth', 'FID50KFullDepth']

FEATURE_DIM = 2048  # Dimension of inception feature.


class FIDMetricDepth(BaseGANMetric):
    """Defines the class for FID metric computation on depth images."""
    ...
    def extract_real_features(self, data_loader):
        ...
        cache_name = f'{dataset_name}_{real_num}_inception_feature_depth.npy'
        ...
        for batch_idx in range(len(data_loader)):
            if batch_idx * batch_size >= replica_num:
                ...
                continue
            with torch.no_grad():
                batch_data = next(data_loader)['image'][:,3].unsqueeze(1).repeat(1,3,1,1).cuda().detach()
    ...
    def extract_fake_features(self, generator, generator_kwargs, generator_depth, generator_depth_kwargs):
        ...
        for start in range(0, self.replica_latent_num, batch_size):
            ...
            with torch.no_grad():
                ...
                batch_images = G_d(batch_codes_d, batch_codes_angles, batch_labels, **G_d_kwargs)['image'].repeat(1,3,1,1)
...
class FID50KDepth(FIDMetricDepth):
...
class FID50KFullDepth(FIDMetricDepth):
