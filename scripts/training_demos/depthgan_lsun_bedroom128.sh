...
# Help message.
if [[ $# -lt 2 ]]; then
    echo "This script launches a job of training DepthGAN on LSUN-Bedroom-128."
    echo
    ...
    echo
    echo "Example: $0 8 /data/LSUN/bedroom_train_rgbd.zip [--help]"
    echo
    exit 0
fi
...
./scripts/dist_train.sh ${GPUS} depthgan \
    --job_name='depthgan_lsun_bedroom128' \
    --seed=0 \
    --resolution=128 \
