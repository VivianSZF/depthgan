...
# Help message.
if [[ $# -lt 2 ]]; then
    echo "This script launches a job of training StyleGAN2 on LSUN-Bedroom-256."
    echo
    ...
    echo
    echo "Example: $0 8 /data/LSUN/bedroom_train_lmdb [--help]"
    echo
    exit 0
fi
...
./scripts/dist_train.sh ${GPUS} stylegan2 \
    --job_name='stylegan2_lsun_bedroom256' \
    --seed=0 \
    --resolution=256 \
    --image_channels=3 \
