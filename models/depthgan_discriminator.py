# python3.7
"""Contains the implementation of discriminator described in DepthGAN.
...
class DepthGANDiscriminator(nn.Module):
    """Defines the discriminator network in DepthGAN.
    ...
    (8) mbstd_group: Group size for the minibatch standard deviation layer.
    ...
    (1) label_size: Size of the additional label for conditional model.
        (default: 0)
    ...
    (1) impl: Implementation mode of some particular ops, e.g., `filtering`,
        `bias_act`, etc. `cuda` means using the official CUDA implementation
        from StyleGAN2, while `ref` means using the native torch ops.
        (default: `cuda`)
    (2) fp16_res: Layers at resolution higher than (or equal to) this field will
    ...
    def __init__(self,
        ...
        self.use_embedding = label_size > 0 and embedding_dim > 0
        ...
        self.add_module(
                    'input_depth',
                    ConvLayer(in_channels=1,
                              out_channels=self.get_nf(2**self.final_res_log2),
                              kernel_size=1,
                              add_bias=True,
                              scale_factor=1,
                              filter_kernel=None,
        ...
        for res_log2 in range(self.final_res_log2, self.init_res_log2 - 1, -1):
            ...
            if res != self.init_res:
                ...
                # Residual branch (kernel 1x1) with dowsampling, without bias,
            ...
            # Convolution block for last resolution.
                ...
                self.mbstd = MiniBatchSTDLayer(
                    group=mbstd_group, new_channels=mbstd_channels, eps=eps)
        ...
        self.depthlosslayer = DepthLossLayer(self.get_nf(64), self.get_nf(32), self.get_nf(16), self.num_classes, self.resolution//64)

    def get_nf(self, res):
        """Gets number of feature maps according to current resolution."""
        return min(self.fmaps_base // res, self.fmaps_max)

    def forward(self, image, label=None, impl='cuda', fp16_res=None):
        # Check shape.
        expected_shape = (self.resolution, self.resolution)
        if image.ndim != 4 or image.shape[2:] != expected_shape or image.shape[1] not in [1,3,4]:
            raise ValueError(f'The input tensor should be with shape '
                             f'[batch_size, channel, height, width], where '
                             f'`channel` equals to {self.image_channels}, '
                             f'`height`, `width` equal to {self.resolution}!\n'
                             f'But `{image.shape}` is received!')
        batch_s, channel, height, width = image.shape
        ...
        if channel == 3:
            add_channel = torch.zeros(batch_s, 1, height, width).to(image.device)
            x = self.input0(image, impl=impl) + self.input_depth(add_channel, impl=impl) * 0
        else:
            
            x = self.input_depth(image[:,3:], impl=impl) + self.input0(image[:,:3], impl=impl)

        for res_log2 in range(self.final_res_log2, self.init_res_log2, -1): ##?????
            res = 2 ** res_log2
            if res ==64 or res == 32 or res==16:
        ...
        upsampleddepth = self.depthlosslayer(feature_list[0], feature_list[1], feature_list[2])
        results['depthimage'] = upsampleddepth
        if self.use_embedding:
...
class MiniBatchSTDLayer(nn.Module):
    ...
    def __init__(self, group, new_channels, eps):
        super().__init__()
        self.group = group
    ...
    def forward(self, x):
        if self.group <= 1 or self.new_channels < 1:
            return x

        dtype = x.dtype

        N, C, H, W = x.shape
        G = min(self.group, N)  # Number of groups.
...
class ResBlock(torch.nn.Module):
    ...
    def __init__(self, n_filters=8):
        super().__init__()
        self.conv1 = ConvLayer(in_channels=n_filters,
                               ...
                               activation_type='lrelu',
                               conv_clamp=None)
        self.conv2 = ConvLayer(in_channels=n_filters,
                               out_channels=n_filters,
                               kernel_size=3,
                               add_bias=True,
                               scale_factor=1,
