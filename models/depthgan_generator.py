# python3.7
"""Contains the implementation of generator described in DepthGAN.
...
__all__ = ['DepthGANGenerator_depth', 'DepthGANGenerator_rgb']
...
class DepthGANGenerator_depth(nn.Module):
    """Defines the depth generator network in DepthGAN.
    ...
    def __init__(self,
        ...
        self.synthesis = SynthesisNetwork_depth(resolution=resolution,
                                          init_res=init_res,
                                          w_dim=w_dim,
    ...
    def forward(self,
        ...
        mapping_results = self.mapping(z, label, impl=impl, a=a)

        w = mapping_results['w']
        cam_p = mapping_results['cam_p']
        ...
        if not self.training:
            ...
            elif trunc_psi < 1.0 and trunc_layers == 0:
                w_avg = self.w_avg.reshape(1, -1, self.w_dim)
                wp = w_avg.lerp(wp, trunc_psi)
...
class DepthGANGenerator_rgb(nn.Module):
    """Defines the RGB generator network in DepthGAN.
    ...
    (1) z_dim: Dimension of the input latent space, Z. (default: 512)
    (2) w_dim: Dimension of the outout latent space, W. (default: 512)
    (3) repeat_w: Repeat w-code for different layers. (default: True)
    (4) normalize_z: Whether to normalize the z-code. (default: True)
    (5) mapping_layers: Number of layers of the mapping network. (default: 8)
    (6) mapping_fmaps: Number of hidden channels of the mapping network.
    ...
    (1) w_moving_decay: Decay factor for updating `w_avg`, which is used for
        training only. Set `None` to disable. (default: 0.995)
    (2) sync_w_avg: Synchronizing the stats of `w_avg` across replicas. If set
        as `True`, the stats will be more accurate, yet the speed maybe a little
        bit slower. (default: False)
    (3) style_mixing_prob: Probability to perform style mixing as a training
        regularization. Set `None` to disable. (default: 0.9)
    (4) trunc_psi: Truncation psi, set `None` to disable. (default: None)
    (5) trunc_layers: Number of layers to perform truncation. (default: None)
    (6) noise_mode: Mode of the layer-wise noise. Support `none`, `random`,
        `const`. (default: `const`)
    ...
    def __init__(self,
        ...
        self.synthesis = SynthesisNetwork_rgb(resolution=resolution,
                                          init_res=init_res,
                                          w_dim=w_dim,
                                          image_channels=image_channels,
                                          final_tanh=final_tanh,
...
class MappingNetwork(nn.Module):
    ...
    def __init__(self,
        ...
        if self.angle:
            self.params_fc1 = DenseLayer(in_channels=8,
                                         ...
                                         lr_mul=lr_mul,
                                         activation_type='lrelu')
            self.params_fc2 = DenseLayer(in_channels=output_dim,
                                         out_channels=output_dim,
                                         add_bias=True,
                                         init_bias=0.0,
                                         use_wscale=use_wscale,
    ...
    def forward(self, z, label=None, impl='cuda', a=None):
        ...
        cam_p = None
        if self.angle:
            input_a = torch.cat([torch.sin(a), torch.cos(a), torch.sin(2*a), torch.cos(2*a), torch.sin(3*a), torch.cos(3*a), torch.sin(4*a), torch.cos(4*a)], dim=-1)
            cam_ = self.params_fc1(input_a)
            cam_ = self.params_fc2(cam_)
        ...
        if self.num_outputs is not None:
            if self.repeat_output:
                ...
                if cam_ is not None:
                    cam_p = cam_.unsqueeze(1).repeat((1, 3, 1))
            else:
                wp = w.reshape(-1, self.num_outputs, self.output_dim)
                if cam_ is not None:
                    cam_p = cam_.reshape(-1, 3, self.output_dim)
...
class SynthesisNetwork_depth(nn.Module):
            ...
            for layer_idx in range(self.num_layers - 1):
                ...
                if layer_idx in [0,1,2]:
                    ...
                    x, style = layer(x,
                                    ...
                                    impl=impl)
                results[f'style{layer_idx}'] = style
                results[f'output_feature{layer_idx}'] = x
                if layer_idx % 2 == 0:
                    output_layer = getattr(self, f'output{layer_idx // 2}')
...
class SynthesisNetwork_rgb(nn.Module):
    """Implements the image synthesis network.
        ...
        for res_log2 in range(self.init_res_log2, self.final_res_log2 + 1):
            ...
            if res == init_res:
                ...
                self.add_module(
                    ...
                    ModulateConvLayer(in_channels=in_channels,
                                      ...
                                      conv_clamp=conv_clamp,
                                      eps=eps))
                d_layer_gamma = f'd_layer_gamma{2 * block_idx - 1}'
                d_layer_beta = f'd_layer_beta{2 * block_idx - 1}'
                self.add_module(
                    d_layer_gamma,
                    ConvLayer(in_channels=out_channels*2,
                                out_channels=out_channels,
                                kernel_size=1,
                                add_bias=True,
                                scale_factor=1,
            ...
            self.add_module(
                ...
                ModulateConvLayer(in_channels=out_channels,
                                  ...
                                  conv_clamp=conv_clamp,
                                  eps=eps))
            d_layer_gamma = f'd_layer_gamma{2*block_idx}'
            d_layer_beta = f'd_layer_beta{2*block_idx}'
            self.add_module(
                d_layer_gamma,
                ConvLayer(in_channels=out_channels*2,
                            out_channels=out_channels,
    ...
    def forward(self,
        ...
        if self.architecture == 'origin':
            ...
            for layer_idx in range(self.num_layers - 1):
                ...
                d_layer_gamma = self.__getattr__(f'd_layer_gamma{layer_idx}')
                d_layer_beta = self.__getattr__(f'd_layer_beta{layer_idx}')

                condition_input = torch.cat((x, result_d[f'output_feature{layer_idx}']), dim=1)
                condition_output = d_layer_gamma(condition_input)
                x = d_layer_beta(condition_output)
...
class InputLayer(nn.Module):
    ...
    def forward(self, w, **_unused_kwargs):
...
class ConvLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if use_wscale:
            self.weight = nn.Parameter(torch.randn(*weight_shape) / init_cov / lr_mul)
            self.wscale = wscale * lr_mul
        else:
            self.weight = nn.Parameter(
                torch.randn(*weight_shape) * wscale / init_cov / lr_mul)
            self.wscale = lr_mul
...
class DenseLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if add_bias:
            init_bias = np.float32(init_bias)
