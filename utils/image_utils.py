...
def get_blank_image(height, width, channels=3, use_black=True):
    ...
    if channels == 3:
        ...
        return np.ones(shape, dtype=np.uint16) * 60000
...
def postprocess_image(image, min_val=-1.0, max_val=1.0):
    ...
    if channel == 1:
        image = image.astype(np.float64)
        image = (image - min_val) / (max_val - min_val) * 60000
        image = np.clip(image + 0.5, 0, 60000).astype(np.uint16)
    ...
    if channel == 3:
        ...
        image = image.astype(np.float64)
        rgb = image[:,:3]
        depth = np.expand_dims(image[:,3],axis=1)
        rgb = (rgb - min_val) / (max_val - min_val) * 255
        rgb = np.clip(rgb + 0.5, 0, 255).astype(np.uint8)

        depth = (depth - min_val) / (max_val - min_val) * 60000
        depth = np.clip(depth + 0.5, 0, 60000).astype(np.uint16)

        # depth = (depth - min_val) / (max_val - min_val) * 255
        # depth = np.clip(depth + 0.5, 0, 255).astype(np.uint8)

        assert rgb.ndim == 4 and depth.ndim == 4
        return rgb.transpose(0, 2, 3, 1), depth.transpose(0, 2, 3, 1)
